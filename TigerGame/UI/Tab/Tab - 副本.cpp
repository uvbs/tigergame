...
{
	CString str;
	str.Format(_T("Ñ¡Ôñ=%d"), m_nDownItem);
	AfxMessageBox(str);
	return false;
}
...
void CTab::DrawControl(CDC &dc, CRect rcParent, CRect rcUpdate)
{
		...
		for(int i = 0; i < 3; i++)
		{
			...
			for(size_t j = 0; j < m_vecItemInfo.size(); j++)
			{
				...
				if (j < m_vecItemInfo.size() - 1 && m_ImageSeperator.GetImage() != NULL)
				{
					...
					graphics.DrawImage(m_ImageSeperator.GetImage(), Rect(nXPos, nYPos, sizeSeperator.cx, sizeSeperator.cy),
	...
	for (size_t i = 0; i < m_vecItemInfo.size(); i++)
	{
		...
		if (m_nHoverItem == i)
		{
			DrawControlEx(dc, rcParent, rcUpdate, itemInfo.rc.left, itemInfo.rc.top + m_rc.Height());
		}
		else if (m_nDownItem == i)
		{
			DrawControlEx(dc, rcParent, rcUpdate, itemInfo.rc.left, itemInfo.rc.top + m_rc.Height() * 2);
		}
		else
		{
			DrawControlEx(dc, rcParent, rcUpdate, itemInfo.rc.left, itemInfo.rc.top);
		}
	}
}