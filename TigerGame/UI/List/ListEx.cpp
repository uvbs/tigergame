...
bool CListEx::SetControlRect(CRect rc)
{
	...
	m_nControlHeight = __max(m_rc.Height(), m_nItemHeight * m_vecListInfo.size());
...
void CListEx::DrawControl(CDC &dc, CRect rcParent, CRect rcUpdate)
{
	...
	if (m_bUpdate)
	{
		...
		for (size_t i = nStartIndex; i < m_vecListInfo.size() && i < nStartIndex + nItemCount; i++)
		{
			ListInfo &listInfoTemp = m_vecListInfo.at(i);

			graphics.FillRectangle(i % 2 == 0 ? &solidBrush1 : &solidBrush2, 0, nYPos, nWidth, m_nItemHeight);

			CString strItemText = listInfoTemp.strText;
...
int CListEx::AddItem(int nItem, CString strItemText, CString strItemValue)
{
	//m_vecItemText.push_back(strItemText);
	ListInfo ListInfo;
	ListInfo.strText = strItemText;
	ListInfo.strValue = strItemValue;

	if (nItem <= -1 || nItem >= m_vecListInfo.size())
	{
		m_vecListInfo.push_back(ListInfo);
	}
	else
	{
		m_vecListInfo.insert(m_vecListInfo.begin() + nItem, ListInfo);
	}

	UpdateScroll();

	return m_vecListInfo.size();
}
...
bool CListEx::DeleteItem(UINT uID)
{
	bool bRet = false;
	int i = 0;
	vector<ListInfo>::iterator it;
	for (it = m_vecListInfo.begin(); it != m_vecListInfo.end(); it++)
	{
		if ( i == uID)
		{
			it = m_vecListInfo.erase(it); // 不能写成arr.erase(it);
			bRet = true;
			break;
		}
		else
...
int CListEx::SetSelect(int nPlayItem)
{
	...
	if (nPlayItem < m_vecListInfo.size())
	{
		m_bPlay = true;
...
CString CListEx::GetValue(int nItem)
{
	CString strTet = L"";
...
CString CListEx::GetSelectValue()
{
	return GetValue(m_nSelectItem);
}
...
bool CListEx::OnControlMouseMove(UINT nFlags, CPoint point)
{
	...
	if (m_rc.PtInRect(point))
	{
		...
		if (m_nHoverItem >= m_vecListInfo.size() || m_nPlayItem == m_nHoverItem ||m_nHoverItem == m_nSelectItem)
		{
			m_nHoverItem = -1;
		}
	}
