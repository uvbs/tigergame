...
Color g_clrSkin[12 * 7] = {Color(254, 126, 0, 0), Color(254, 132, 74, 0), Color(254, 116, 103, 0), Color(254, 72, 106, 0), Color(254, 0, 92, 86), Color(254, 11, 75, 108),
					Color(254, 0, 49, 88), Color(254, 1, 0, 76), Color(254, 47, 0, 74), Color(254, 80, 0, 64), Color(254, 112, 1, 53), Color(254, 0, 0, 0),
					...
					Color(254, 137, 171, 218), Color(254, 141, 151, 202), Color(254, 169, 137, 188), Color(254, 241, 158, 187), Color(254, 241, 156, 156), Color(254, 255, 255, 255)};
...
void CDlgSkin::InitUI(CRect rcClient)
{
	...
	nYStartButton = 45;
	rcTemp.SetRect(nXStartButton, nYStartButton, nXStartButton + 205, nYStartButton + 127);
	pControlBase = new CSelectBox(GetSafeHwnd(),this, BOX_IMAGE, rcTemp, 3, 3, true, Color(255, 255, 255, 255), Color(128, 0, 0, 0), Color(255, 56, 158, 221));
	((CSelectBox *)pControlBase)->SetBitmap(IDB_SKIN_PIC_SMALL_0, Color(255, 224, 107, 87));
	((CSelectBox *)pControlBase)->SetBitmap(IDB_SKIN_PIC_SMALL_1, Color(255, 8, 194, 215));
	((CSelectBox *)pControlBase)->SetBitmap(IDB_SKIN_PIC_SMALL_2, Color(255, 169, 190, 23));
	((CSelectBox *)pControlBase)->SetBitmap(IDB_SKIN_PIC_SMALL_3, Color(255, 157, 103, 31));
	((CSelectBox *)pControlBase)->SetBitmap(IDB_SKIN_PIC_SMALL_4, Color(255, 53, 168, 233));
	((CSelectBox *)pControlBase)->SetBitmap(IDB_SKIN_PIC_SMALL_5, Color(255, 115, 89, 176));
	((CSelectBox *)pControlBase)->SetBitmap(IDB_SKIN_PIC_SMALL_6, Color(255, 42, 195, 130));
	((CSelectBox *)pControlBase)->SetBitmap(IDB_SKIN_PIC_SMALL_7, Color(255, 130, 74, 165));
	((CSelectBox *)pControlBase)->SetBitmap(IDB_SKIN_PIC_SMALL_8, Color(255, 58, 106, 178));
	m_vecControl.push_back(pControlBase);	
...
// 消息响应
LRESULT CDlgSkin::OnControlMessage(UINT uControlID, UINT uMsg, void *pData, size_t lSize)
{
	if(uControlID == BT_SKIN_IMAGE)
	{
		if(uMsg == BUTTOM_UP && !m_bImage)
		{
			m_bImage = true;
	...
	else if(uControlID == BT_SKIN_COLOR)
	{
		if(uMsg == BUTTOM_UP && m_bImage)
		{
			m_bImage = false;
	...
	else if(uControlID == BOX_COLOR)
	{
		if(uMsg == BUTTOM_DOWN)
		{
			tagSelectBox *pSelectBox = (tagSelectBox *)pData;
			tagSkinInfo skinInfo;
			skinInfo.nType = pSelectBox->nType;
			skinInfo.clr = pSelectBox->clr;
			skinInfo.uIndex = pSelectBox->uIndex;
			PostMessageEx(MSG_SKIN, &skinInfo, sizeof(skinInfo));
		}
	}
	else if(uControlID == BOX_IMAGE)
	{
		if(uMsg == BUTTOM_DOWN)
		{
			tagSelectBox *pSelectBox = (tagSelectBox *)pData;
	...
	else if(uControlID == BT_SKIN_CUSTOMIZE)
	{
		if(uMsg == BUTTOM_DOWN)
		{
			PostMessageEx(MSG_SELECT_SKIN);
		}
	}
...
// 消息响应
LRESULT CDlgSkin::OnWindowMessage(UINT uControlID, UINT uMsg, void *pData, size_t lSize)
{
	return 0;
}